...
data: [DONE]"#;
    ...
    fn test_get_thinking_config() {
        ...
        // Test 3: Gemini 2.5 model with default thinking budget
        ...
        // Test 4: Gemini 2.5 model with custom thinking budget
        let mut params = HashMap::new();
        params.insert("thinking_effort".to_string(), json!("high"));
        ...
        // Test 5: Gemini 2.5 model with invalid thinking budget falls back to default
        let mut params = HashMap::new();
        params.insert("thinking_effort".to_string(), json!("high"));
        ...
        // Test 6: Gemini 2.5 without explicit thinking_effort still returns thinking config (budget-based)
        let config = ModelConfig::new("gemini-2.5-flash").unwrap();
        let result = get_thinking_config(&config);
        assert!(result.is_some());

        // Test 7: Non-thinking models return None
